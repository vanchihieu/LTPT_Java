...
// WHERE d.speciality = "Dermatology Services"
...
//		return entityManager.createQuery("SELECT p FROM Patient p JOIN p.treatments t JOIN t.doctor d WHERE d.speciality = :speciality AND YEAR(t.startDate) = :year AND MONTH(t.startDate) = :month")
//				.setParameter("speciality", speciality)
//				.setParameter("year", year)
//				.setParameter("month", month)
//				.getResultList();
		return entityManager.createNativeQuery("SELECT * FROM patients p JOIN treatments t ON p.person_id = t.patient_id JOIN doctors d ON t.doctor_id = d.person_id WHERE d.speciality = :speciality AND YEAR(t.start_date) = :year AND MONTH(t.start_date) = :month", Patient.class)
				.setParameter("speciality", speciality)
				.setParameter("year", year)
				.setParameter("month", month)
				.getResultList();
...
//	GROUP BY d.speciality
	...
	public Map<String, Long> getNoOfPatientsBySpeciality(String departmentName) throws RemoteException {
		List<Object[]>result = entityManager.createNativeQuery("SELECT d.speciality, COUNT(p.person_id) FROM patients p JOIN treatments t ON p.person_id = t.patient_id JOIN doctors d ON t.doctor_id = d.person_id JOIN departments dp ON d.department_id = dp.department_id WHERE dp.name = :departmentName GROUP BY d.speciality", Patient.class)
